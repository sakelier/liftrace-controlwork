...
void LLController::load_params() {
    ...
    if (!std::isfinite(drop_height_threshold) ||
        ...
        external_standard_recovery_setpoint_height_ > 4.0 ||
        external_cross_recovery_setpoint_height_ < external_recovery_height_ ||
        external_cross_recovery_setpoint_height_ > 4.0) {
    ...
    ROS_INFO("[UavVision] external recovery handoff/setpoints: %.2f / %.2f / %.2f m",
             external_recovery_height_,
             external_standard_recovery_setpoint_height_,
             external_cross_recovery_setpoint_height_);
    ROS_INFO(
        ...
        if(have_waypoint_mark && dynamic_record_activate && !dynamic_first_flag)
        {
            ...
            if(count_aligning >= 50){
                ...
                if(servo_complete.data){
                    ...
                    down_flag = false;
                    align_height = external_mission_mode_
                        ? external_cross_recovery_setpoint_height_ : 1.15;
    ...
    if (patrol_control::alignmentWindowOpen(
        ...
        if(count_aligning >= required_alignment_samples && align_ok == 1){
            ...
            if(servo_complete.data){
                ...
                // down_flag = false;
                align_height = external_mission_mode_
                    ? external_standard_recovery_setpoint_height_ : 1.2;
                const double recovery_height = external_mission_mode_
...
void LLController::cleanupAfterCrossDrop() {
    ...
    // 重置高度；外部任务回到 YAML 配置的下一次对准高度。
    align_height = external_mission_mode_
        ? external_alignment_capture_height_ : 1.2;
