#!/usr/bin/env bash
# Read-only ROS recorder. Does not start drivers, mission nodes or control services.
set -euo pipefail
...
set -u
for cmd in rosbag rosparam rosnode rostopic timeout; do
...
RUN_DIR="$(mktemp -d "$OUTPUT_PARENT/corridor_diag_$(date +%Y%m%d_%H%M%S)_XXXXXX")"
...
printf '%s\n' "${TOPICS[@]}" > "$RUN_DIR/topics.txt"
{
  ...
  echo 'Point clouds, images and actuator topics are excluded.'
...
snapshot() {
  ...
  for key in /use_sim_time /waypoints /navigation/mission_manager/mission \
    ...
    /fast_planner_node/sdf_map /external_mission_mode /external_planner_max_command_z \
...
rosbag record --buffsize=64 --split --size=128 --repeat-latched -O "$RUN_DIR/navigation.bag" \
